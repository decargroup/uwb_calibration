...
for lv0 in range(1, n):
    ...
    delta[lv0] = (
        delta[lv0 - 1]
        + T / 1e9 * gamma[lv0 - 1]
        + T / 1e9 * np.random.randn() * np.math.sqrt(Q_delta)
    )
...
P = np.array([[P0_delta, 0], [0, P0_gamma]])
for lv0 in range(1, n):
    # Prediction
    T = tx1[lv0] - tx1[lv0 - 1]  # note that this now has unmodelled noise!
    A = np.array([[1, T / 1e9], [0, 1]])
    x = A @ x
    P = A @ P @ A.T + (T / 1e9) ** 2 * np.array([[Q_delta, 0], [0, Q_gamma]])

    # Correction
    y_iter = np.array([[y_offset[lv0]], [y_skew[lv0]]])

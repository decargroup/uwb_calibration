...
class PostProcess(object):
    ...
    def _extract_ts_data(self, formation_number, agent_number):
        ...
        for line in Lines:
            if line[0] == "A":
                ...
                ts_data[neighbour] = np.vstack((ts_data[neighbour], temp))
    ...
    def _store_range_meas_mean(self):
        ...
        id1 = self.board_ids[1]
        for lv1 in range(self.num_of_formations):
            self.mean_range_meas[lv1, :] = self._calculate_mean_range(
                self.ts_data[lv1][str(id0)], self.ts_data[lv1][str(id1)]
            )
    ...
    def _match_gt_with_range(self):
        for lv1 in range(self.num_of_formations):
            ...
            dist_matrix = np.fromiter(
                (dist(x1, x2) for x1 in gt_iter for x2 in meas_iter),
                dtype=t,
                count=n * n,
            ).reshape(n, n)
